# Task: make the Saved/error toggle messages mutually exclusive

Small fix flagged in review: toggleSaved and toggleError were
independent booleans, so a success immediately followed by a failure
could show "Saved" and the error message together for the remainder of
the 1800ms Saved timer. Added setToggleSaved(false) in the catch block.
The reverse case was already covered — setToggleError('') already runs
at the start of every toggle attempt, so a new success already clears
any lingering error from a previous failure.

This task has ONE file to write.

## Write file: `frontend/src/pages/EmergencyCard.tsx`
```tsx
...
```
grep -n "setToggleSaved(false)" frontend/src/pages/EmergencyCard.tsx
```
Must show a match inside the catch block (not just the initial useState
declaration).
...
Report the grep check and build output before saying the task is
complete.
